...
const MealForm: any = ({ isUpdate = false, attendanceId = '' }) => {
  ...
  const handleSaveAll = async () => {
    try {
      // If there are changes, save only the changes
      const changes = Object.values(attendanceChanges);
      ...
      if (changes.length > 0) {
        ...
        // No changes made - save ALL default data (all meals present for all students for all dates)
      ...
      setSnackbar({
        open: true,
        message: `${result.totalProcessed || attendancesToSave.length} records saved`,
        severity: 'success'
      });
